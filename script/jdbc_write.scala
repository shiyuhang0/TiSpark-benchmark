...
val df = spark.read.format("csv").option("delimiter","|").schema(schema).load("hdfs://${ip:port}/test1/orders.tbl")
val startTime = System.currentTimeMillis()
df.write.format("jdbc").option("driver", "com.mysql.jdbc.Driver").option("url", "jdbc:mysql://${ip:port}/test?rewriteBatchedStatements=true").option("useSSL", "false").option("dbtable", "ORDERS_J").option("isolationLevel", "NONE").option("user", "root").mode(saveMode = "append").save()
